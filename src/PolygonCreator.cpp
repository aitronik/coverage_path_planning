...
/*************************************/
CGAL::Polygon_2<K> PolygonCreator::deleteIdenticalSegments(CGAL::Polygon_2<K> polygon){

    vector<K::Segment_2> polySegmentsTmp;
    polySegmentsTmp.resize(polygon.size() - 1);
    polySegmentsTmp[0] = K::Segment_2(polygon.vertex(0), polygon.vertex(1));
    
    vector<K::Segment_2> polySegments;
    polySegments.push_back(polySegmentsTmp[0]);

    for(int i = 1; i < polygon.size() - 1; i++){
        polySegmentsTmp[i] = K::Segment_2(polygon.vertex(i), polygon.vertex(i+1));
        if(polySegmentsTmp[i-1].operator!=(polySegmentsTmp[i].opposite()) && sqrt(polySegmentsTmp[i].squared_length()) > 0.0001){
            polySegments.push_back(polySegmentsTmp[i]);
        }
    }

    CGAL::Polygon_2<K> polygonOut;
    polygonOut.push_back(polySegments.at(0).point(0));

    for(int i = 0; i < polySegments.size(); i++){
        polygonOut.push_back(polySegments.at(i).point(1));
    }
...
/*************************************/
vector<CGAL::Polygon_2<K2>> PolygonCreator::createArrangement(CGAL::Polygon_with_holes_2<K2> polygon){

    polygon.outer_boundary().push_back(polygon.outer_boundary().vertex(0));

    vector<PointT> polygonTraits;
    polygonTraits.resize(polygon.outer_boundary().size());

    vector<SegmentT> segmentTraits;
    segmentTraits.resize(polygon.outer_boundary().size()-1);

    polygonTraits[0] = {CGAL::to_double(polygon.outer_boundary().vertex(0).x()), CGAL::to_double(polygon.outer_boundary().vertex(0).y())};

    for(int i = 1; i < polygon.outer_boundary().size(); i++){
        polygonTraits[i] = PointT(CGAL::to_double(polygon.outer_boundary().vertex(i).x()), CGAL::to_double(polygon.outer_boundary().vertex(i).y()));
        segmentTraits[i-1] = SegmentT(polygonTraits[i-1], polygonTraits[i]);
    }


    ArrangementT arr;
    CGAL::insert(arr, segmentTraits.cbegin(), segmentTraits.cend());
    vector<CGAL::Polygon_2<K2>> polygons;
...
/*************************************/
vector<CGAL::Polygon_with_holes_2<K2>> PolygonCreator::simplifyPolygon3(CGAL::Polygon_with_holes_2<K2> polygon){
    
    vector<CGAL::Polygon_2<K2>> polygons = createArrangement(polygon);

    cout << polygons.size();
    
    vector<CGAL::Polygon_2<K2>> polygonsSimpleFinal;
    vector<CGAL::Polygon_with_holes_2<K2>> polygonsWithHoles;
    for(int i = 0; i < polygons.size(); i++){
        
        vector<CGAL::Polygon_2<K2>> polygonsSimple;
        if(polygons.at(i).is_simple() == false){
            CGAL::Polygon_with_holes_2<K2> polyWithHolesTmp(convertPoly2K2(deleteIdenticalSegments(convertPoly2K(polygons.at(i)))));
            polygonsSimple = createArrangement(polyWithHolesTmp);
        }
        else{
            polygonsSimple.push_back(polygons.at(i));
        }

        cout << "prova" << endl;

            for(int k = 0; k < polygonsSimple.size(); k++){

                vector<CGAL::Polygon_2<K2>> polygonsSimple2;
                if(polygonsSimple.at(k).is_simple() == false){
                    CGAL::Polygon_2<K> pp = deleteIdenticalSegments(convertPoly2K(polygonsSimple.at(k)));
                    CGAL::Polygon_with_holes_2<K2> polyWithHolesTmp2(convertPoly2K2(pp));
                    polygonsSimple2 = createArrangement(polyWithHolesTmp2);
                }
                else{
                    polygonsSimple2.push_back(polygonsSimple.at(k));
                }

                for(int j = 0; j < polygonsSimple2.size(); j++){
                    polygonsSimpleFinal.push_back(polygonsSimple2.at(j));
                }
    ...
    for(int s = 0; s < polygonsSimpleFinal.size(); s++){

        CGAL::Polygon_with_holes_2<K2> polyWithHolesTmp3(polygonsSimpleFinal.at(s));
        for(int j = 0; j < polygon.number_of_holes(); j++){
            
            if(polygon.holes().at(j).is_clockwise_oriented() == true){
                polygon.holes().at(j).reverse_orientation();
            }  
            
            auto ss = CGAL::oriented_side(polygon.holes().at(j), polygonsSimpleFinal.at(s));
            if(ss == CGAL::POSITIVE){
                polyWithHolesTmp3.add_hole(polygon.holes().at(j));
            }  
        }

        polygonsWithHoles.push_back(polyWithHolesTmp3);
    }
...
/*************************************/
CGAL::Polygon_with_holes_2<K2> PolygonCreator::mergePolygons(vector<pair<CGAL::Polygon_with_holes_2<K>, int>>& polygonUnion){

    int maxArea = 0;
    int indexMaxArea;
    for(int i = 0; i < polygonUnion.size(); i++){
        if(abs(polygonUnion.at(i).first.outer_boundary().area()) > maxArea){
            maxArea = abs(polygonUnion.at(i).first.outer_boundary().area());
            indexMaxArea = i;
        }
    }

    CGAL::Polygon_with_holes_2<K2> firstPolygon = convertPolyWithHoles2K2(polygonUnion.at(indexMaxArea).first);
    CGAL::Polygon_with_holes_2<K2> merge = firstPolygon;

    while(polygonUnion.size() > 0){
        for(int j = 0; j < polygonUnion.size(); j++){
            if(polygonUnion.at(j).first.outer_boundary().size() != 0){
                if(CGAL::do_intersect(convertPolyWithHoles2K2(polygonUnion.at(j).first), merge) == true){
                    CGAL::join(merge, convertPolyWithHoles2K2(polygonUnion.at(j).first), merge);
                    polygonUnion.erase(polygonUnion.begin() + j);
                    break;
                }
            }
            else{
...
pair<Polygon_list, vector<K::Point_2>> PolygonCreator::decomposeContour(pair<CGAL::Polygon_with_holes_2<K>, int> contour){
    ...
    m_cpc.init(contour.first.outer_boundary(), 0.4, 1);
    m_cpc.run();
    ...
    // m_cph.printPolygonsWithHolesK(polyWithholesTmpFin, m_cph.m_decomposedPolys, false, 0, 0, 0, 255, "Decomposed Polygons", false, true, false, clnLvlsFin);
...
/*************************************/
void PolygonCreator::defineOptimalPath(vector<pair<CGAL::Polygon_with_holes_2<K>, int>> polygonUnionFinalNoOverlaps2){

    vector<K::Point_2> finalPathCGAL;
    // finalPathCGAL.resize(m_cpc.m_pathToReturn.size());

    vector<int> cleaningLvl;
    // cleaningLvl.resize(m_cpc.m_pathToReturn.size());

    vector<pair<CGAL::Polygon_with_holes_2<K2>, int>> polygons4ClnLvls;

    cout << polygonUnionFinalNoOverlaps2.size() << endl;

    for(int i = 0; i < polygonUnionFinalNoOverlaps2.size(); i++){
        
        CGAL::Polygon_with_holes_2<K> polyNoDegenerated = deleteDegeneratedSegments(polygonUnionFinalNoOverlaps2.at(i).first);
        cout << "ue" << endl;
        vector<CGAL::Polygon_with_holes_2<K2>> polygons;
        if(polyNoDegenerated.outer_boundary().is_simple() == false){
            cout << "ue2" << endl;
            polygons = simplifyPolygon3(convertPolyWithHoles2K2(polyNoDegenerated));
            cout << "ue3" << endl;
            cout << i << endl;
        }
        else{
            polygons.push_back(convertPolyWithHoles2K2(polyNoDegenerated));
        }

        for(int j = 0; j < polygons.size(); j++){

            // if(polygons.at(j).outer_boundary().is_clockwise_oriented() == true){
            //     polygons.at(j).outer_boundary().reverse_orientation();
            // }

            for(int k = 0; k < polygons.at(j).number_of_holes(); k++){
                if(polygons.at(j).holes().at(k).is_clockwise_oriented() == false){
                    polygons.at(j).holes().at(k).reverse_orientation();
                }
            }

            polygons4ClnLvls.push_back({polygons.at(j), polygonUnionFinalNoOverlaps2.at(i).second});
        }
    ...
    for (int i = 0; i < m_cpc.m_pathToReturn.size(); i = i + 2){
        K::Point_2 pathPoint(m_cpc.m_pathToReturn.at(i).x(), m_cpc.m_pathToReturn.at(i).y());
        finalPathCGAL.push_back(pathPoint);
        bool belong = false;
        for(int j = 0; j < polygons4ClnLvls.size(); j++){
            K2::Point_2 pointK2(pathPoint.x(), pathPoint.y());
            auto os = CGAL::oriented_side(pointK2, polygons4ClnLvls.at(j).first);
            if(os == CGAL::POSITIVE || os == CGAL::ON_ORIENTED_BOUNDARY){
                cleaningLvl.push_back(polygons4ClnLvls.at(j).second);
                belong = true;
            }
        }

        if(belong == false){
            cleaningLvl.push_back(4);
        }
    }

    cout << "lol2" << endl;

    m_cph.plotPath(finalPathCGAL, m_cph.m_decomposedPolys, 0, 0, 0, 255, false, "Decomposed Path", false, false, cleaningLvl);

    for(int i = 0; i < polygonUnionFinalNoOverlaps2.size()-1; i++){
        vector<int>cleaningLvlStep(polygonUnionFinalNoOverlaps2.at(i).first.outer_boundary().size(), polygonUnionFinalNoOverlaps2.at(i).second);
        m_cph.printPolygonsWithHolesK(polygonUnionFinalNoOverlaps2.at(i).first, m_cph.m_decomposedPolys, false, 0, 0, 0, 255, "Decomposed Path", false, true, false, cleaningLvlStep);
    }

    vector<int>cleaningLvlFin(polygonUnionFinalNoOverlaps2.at(polygonUnionFinalNoOverlaps2.size()-1).first.outer_boundary().size(), polygonUnionFinalNoOverlaps2.at(polygonUnionFinalNoOverlaps2.size()-1).second);
    m_cph.printPolygonsWithHolesK(polygonUnionFinalNoOverlaps2.at(polygonUnionFinalNoOverlaps2.size()-1).first, m_cph.m_decomposedPolys, false, 0, 0, 0, 255, "Decomposed Path", false, true, true, cleaningLvlFin);
    
    //  vector<pair<CGAL::Polygon_with_holes_2<K>, int>> polygons4ClnLvlsK;
    // polygons4ClnLvlsK.resize(polygons4ClnLvls.size());

    // for(int i = 0; i < polygons4ClnLvls.size()-1; i++){
    //     polygons4ClnLvlsK[i] = make_pair(convertPolyWithHoles2K(polygons4ClnLvls.at(i).first), polygons4ClnLvls.at(i).second);
    // }

    // cout << "ciao" << endl;

    // for(int i = 0; i < polygons4ClnLvlsK.size(); i++){
    //     if(polygons4ClnLvlsK.at(i).first.outer_boundary().size() != 0){
    //         vector<int>cleaningLvlStep(polygons4ClnLvlsK.at(i).first.outer_boundary().size(), polygons4ClnLvlsK.at(i).second);
    //         m_cph.printPolygonsWithHolesK(polygons4ClnLvlsK.at(i).first, m_cph.m_decomposedPolys, false, 0, 0, 0, 255, "Decomposed Path", false, true, true, cleaningLvlStep);
    //     }
    // }

    // cout << "ciao2" << endl;

    // vector<int>cleaningLvlFin(polygons4ClnLvlsK.at(polygons4ClnLvlsK.size()-1).first.outer_boundary().size(), polygons4ClnLvlsK.at(polygons4ClnLvlsK.size()-1).second);
    // m_cph.printPolygonsWithHolesK(polygons4ClnLvlsK.at(polygons4ClnLvlsK.size()-1).first, m_cph.m_decomposedPolys, false, 0, 0, 0, 255, "Decomposed Path", false, true, true, cleaningLvlFin);
...
pair<vector<pair<CGAL::Polygon_with_holes_2<K>, int>>, CGAL::Polygon_with_holes_2<K2>> PolygonCreator::createPolygonFromPath(const vector<K::Point_2>& path){
    ...
    CGAL::Polygon_with_holes_2<K2> contour = mergePolygons(polygonUnion);

    cout << "Print Contour" << endl;
    ...
    defineOptimalPath(polygonUnionFinalNoOverlaps2);
