...
bool CoveragePathCreator::init(CGAL::Polygon_2<K> polygon, float sweepDistance, int decompositionType)
{

    // vector<K::Point_2> perimeter_vertices;
    for (size_t i = 0; i < polygon.size(); i++) {
        m_initialPerimeterVertices.push_back(polygon.vertex(i));
    }
...
pair<Polygon_list, vector<K::Point_2>> CoveragePathCreator::decompose(vector<K::Point_2> polyPoints, int decompositionType) {

    cout << "decompose()" << endl;

    Partition_traits_2 traits(CGAL::make_property_map(polyPoints));
    Polygon tmp_polygon;
    ...
    Polygon_list decomposedPolysOfIndices;
    vector<K::Point_2> decomposedVertices;
    ...
    switch (decompositionType) {
        case 0: {
            ...
            CGAL::optimal_convex_partition_2(tmp_polygon.vertices_begin(),
                                            ...
                                            back_inserter(decomposedPolysOfIndices),
                                            traits);
            m_decompositionName = "Optimal convex partition";
            decomposedVertices = polyPoints; //i vertici in questo caso di decomposizione restano gli stessi 
            break;
        } 
        ...
        case 1: { //mydecomposition 
            ...
            pair<Polygon_list, vector<K::Point_2> > p = m_decomposer.run();
            decomposedPolysOfIndices = p.first;
            decomposedVertices = p.second;
    ...
    //     if(poly.size() >= 3){
    //         m_decomposedPolysOfIndices.push_back(poly);
    //         for(int i = 0; i < poly.size(); i++){
    //              m_decomposedVertices.push_back(m_decomposedVertices.at(poly.vertex(i)));
    //         }
    //     }
    // }
    
    m_decomposedPolysOfIndices = decomposedPolysOfIndices;
    m_decomposedVertices = decomposedVertices;
    m_NumerOfDecomposedSubPolygon = m_decomposedPolysOfIndices.size();

    // for(auto it = decomposedPolysOfIndices.begin(); it != decomposedPolysOfIndices.end(); it++){
    //     if(it->edges().size() >= 3){
    //         m_decomposedPolysOfIndices.push_back(*it);
    //     }
    // }
    ...
    return {decomposedPolysOfIndices, decomposedVertices};
}
...
void CoveragePathCreator::createAdjMatrix() {
    ...
    size_t N = m_NumerOfDecomposedSubPolygon; 
    m_adj.resize(N);
...
vector<CGAL::Line_2<K>> CoveragePathCreator::createGrid(CGAL::Segment_2<K> parallelEdge, K::Point_2 point) {
    ...
    for (size_t i = 0; i < intersectionPoints.size(); i++) {
        ...
        CGAL::Line_2<K> l(intersectionPoints[i], parallelEdge.supporting_line().direction());
        grid.push_back(l);
...
vector<K::Point_2> CoveragePathCreator::generateGridForOnePolygon(shared_ptr<CGAL::Polygon_2<K>> polygon, vector<bool> &borders) {
    ...
    if(polygon->edges().size() >= 3){
            ...
            if(restrictedPolygon->edges().size() >= 3){
            ...
            // intersezioni tra le sweep lines e il poligono
            ...
            //intersections.push_back(restrictedPolygon->vertex(0));
        }
    }
    else{
        //intersections.push_back(polygon->vertex(0));
    }    
...
vector<K::Point_2> CoveragePathCreator::simplifyPerimeter(CGAL::Polygon_2<K> poly){

    cout <<"simplifyPerimeter()"<< endl; 

    shared_ptr<CGAL::Polygon_2<K>> simplyfiedPolygon;
    vector<K::Point_2> simplyfiedVertices;

    PS::Squared_distance_cost cost;
    CGAL::Polygon_2<K> approx = PS::simplify(poly, cost, Stop(1));
    simplyfiedPolygon = make_shared<CGAL::Polygon_2<K>>(approx);
    K::Point_2 firstVertexOld = simplyfiedPolygon->vertex(0); 
    simplyfiedPolygon->erase(simplyfiedPolygon->vertices_begin()); //tolgo il vertice iniziale 
    //lo metto come ultimo 
    simplyfiedPolygon->push_back(firstVertexOld); 
    //approssimo nuovamente 
    approx = PS::simplify(*simplyfiedPolygon, cost, Stop(1));
    simplyfiedPolygon = make_shared<CGAL::Polygon_2<K>>(approx);

    // salvo i nuovi vertici
    for (size_t i = 0; i < simplyfiedPolygon->vertices().size(); i++)
    {
        simplyfiedVertices.push_back(simplyfiedPolygon->vertex(i));
    }

    m_simplyfiedPolygon = simplyfiedPolygon;
    m_simplyfiedVertices = simplyfiedVertices;

    return simplyfiedVertices;
...
bool CoveragePathCreator::run() {
    ...
    vector<K::Point_2> polygonPoints = simplifyPerimeter(*m_initialPolygon);
    ...
    m_firstVertex = m_simplyfiedVertices.at(0);
