...
// vector<Point> divideSegment(Segment segment, float range){
//     float length = sqrt(segment.squared_length());
//     cout << "length segment : " << length << endl;
//     cout << "range: " << range << endl;
//     int num_points = length/range +1 ; //intero inferiore ==> l'ultimo pezzo sarà più lungo AND 5 pezzi ==> 6 punti 
...
//         Point* tmp = findPointsAtDistance(m, q, points[i-1], range);
//         //ne trova 2, qual è? 
...
void generatePath(Polygon poly, vector<Point>& final) {
    ...
    float range = 20; //questo range poi va dato dall'esterno 
  //  final = divideSegment(segments.at(0), range);
    for (int i = 0 ; i < segments.size() ; i++) {
        vector<Point> tmp = divideSegment(segments.at(i), range);
